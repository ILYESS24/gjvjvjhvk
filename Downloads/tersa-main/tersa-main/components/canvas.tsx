...
import { parseProjectContent } from "@/types/project";
...
export const Canvas = ({ children, ...props }: ReactFlowProps) => {
  ...
  // Safely parse project content with type validation
  ...
  const save = useDebouncedCallback(async () => {
    ...
    if (!project?.id || !project?.userId) {
      console.warn("[Canvas] Cannot save: missing project ID or user ID");
      return;
    }

    // Additional validation for project ID format
    if (project.id.length < 10) {
      console.error("[Canvas] Invalid project ID format:", project.id);
      return;
    }
