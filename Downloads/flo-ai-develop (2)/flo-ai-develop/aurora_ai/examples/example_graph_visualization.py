#!/usr/bin/env python3
"""
Example script demonstrating the graph visualization feature in Baseaurora.
...
# Create a simple validation tool using the @aurora_tool decorator
@aurora_tool(
...
def main():
    ...
    # Create the Baseaurora instance
    aurora = Baseaurora()
    ...
    aurora.start_at(input_processor)

    # Input processor -> Validation tool
    aurora.add_edge('input_processor', ['validation_tool'])

    # Validation tool -> Analyzer
    aurora.add_edge('validation_tool', ['analyzer'])
    ...
    aurora.add_edge('decision_maker', ['output_formatter'])

    # Output formatter -> End
    aurora.add_end_to(output_formatter)

    # Validate the graph
    try:
        aurora.validate_graph()
    ...
    aurora.visualize_graph('workflow_graph.png')
