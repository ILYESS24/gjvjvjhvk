...
from aurora_ai.tool import aurora_tool, create_tool_from_function
from aurora_ai.models.base_agent import ReasoningPattern
from aurora_ai.llm.openai_llm import OpenAI
from aurora_ai.llm.anthropic_llm import Anthropic
from aurora_ai.llm.base_llm import BaseLLM


# Example 1: Using the @aurora_tool decorator with parameter descriptions
@aurora_tool(
...
# Example 2: Using the @aurora_tool decorator with docstring-based descriptions
@aurora_tool()
...
# Example 3: Using the @aurora_tool decorator with custom name
@aurora_tool(
...
async def test_aurora_tool_decorator():
    """Test the @aurora_tool decorator functionality."""
    print('=== Testing @aurora_tool Decorator ===\n')
...
async def test_multi_tool_agent_with_decorator(llm: BaseLLM, agent_name: str):
    ...
    print(f'\n=== Testing {agent_name} with @aurora_tool decorated functions ===')
...
async def main():
    ...
    await test_aurora_tool_decorator()

    # Test with OpenAI
    openai_llm = OpenAI(model='gpt-40-mini', temperature=0.7)
    await test_multi_tool_agent_with_decorator(openai_llm, 'OpenAI @aurora_tool Agent')

    # Test with Claude
    claude_llm = Anthropic(model='claude-3-5-sonnet-20240620', temperature=0.7)
    await test_multi_tool_agent_with_decorator(claude_llm, 'Claude @aurora_tool Agent')
