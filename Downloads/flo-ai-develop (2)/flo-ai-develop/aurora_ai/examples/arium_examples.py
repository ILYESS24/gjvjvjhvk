"""
Examples demonstrating how to use the auroraBuilder pattern for creating and running aurora workflows.
"""

from typing import Literal
from aurora_ai.aurora import auroraBuilder, create_aurora
from aurora_ai.llm import OpenAI
from aurora_ai.models import TextMessageContent, UserMessage
from aurora_ai.models.agent import Agent
from aurora_ai.aurora.nodes import FunctionNode
from aurora_ai.aurora.memory import MessageMemory, MessageMemoryItem
...
# Example 5: Building and reusing an aurora
async def example_build_and_reuse():
    """Example of building an aurora once and reusing it"""

    agent = Agent(name='echo_agent', prompt='Echo the input')

    # Build the aurora
    aurora = auroraBuilder().add_agent(agent).start_with(agent).end_with(agent).build()

    # Run it multiple times with different inputs
    result1 = await aurora.run(['First input'])
    result2 = await aurora.run(['Second input'])
...
if __name__ == '__main__':
    ...
    # Run the examples
    async def main():
        print('Running auroraBuilder examples...')
