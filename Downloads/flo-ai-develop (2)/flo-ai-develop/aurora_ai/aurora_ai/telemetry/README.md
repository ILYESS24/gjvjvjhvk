...
This module provides comprehensive OpenTelemetry integration for the aurora_ai framework, enabling you to monitor and track:
...
The telemetry dependencies are included in aurora_ai. Install with:

```bash
pip install aurora_ai
```

### 2. Basic Configuration

```python
from aurora_ai import configure_telemetry, shutdown_telemetry
...
```python
configure_telemetry(
    service_name: str = "aurora_ai",              # Service name for telemetry
...
```python
from aurora_ai import get_tracer
...
```python
from aurora_ai import get_meter
...
```python
import atexit
from aurora_ai import shutdown_telemetry
...
- Workflow telemetry with aurora
...
1. Import instrumentation utilities:
   ```python
   from aurora_ai.telemetry.instrumentation import trace_llm_call, agent_metrics
...
This telemetry module is part of aurora_ai and follows the same MIT license.
