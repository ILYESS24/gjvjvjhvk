# aurora Builder Pattern

The `auroraBuilder` provides a fluent, easy-to-use interface for building and running aurora workflows. It eliminates the need for manual graph construction and makes it simple to create complex multi-agent workflows.

## Quick Start

```python
from aurora_ai.aurora import auroraBuilder, create_aurora

# Simple workflow
result = await (auroraBuilder()
                .add_agent(my_agent)
                .add_tool(my_tool)
                .start_with(my_agent)
...
aurora provides comprehensive event monitoring capabilities that allow you to track workflow execution in real-time. This is invaluable for debugging, performance monitoring, and understanding how your workflows behave.
...
```python
from aurora_ai.aurora import auroraBuilder, auroraEventType, default_event_callback

# Enable event monitoring with default callback (logs to console)
aurora = (auroraBuilder()
         .add_agent(my_agent)
         .add_tool(my_tool)
         .start_with(my_agent)
         .connect(my_agent, my_tool)
...
```python
from aurora_ai.aurora import auroraEventType, default_event_callback

# Only monitor workflow lifecycle and node completions
important_events = [
    auroraEventType.WORKFLOW_STARTED,
    auroraEventType.NODE_COMPLETED,
    auroraEventType.WORKFLOW_COMPLETED,
    auroraEventType.WORKFLOW_FAILED
]
...
result = await aurora.run(["Silent execution"])
```

### Event Monitoring vs Build-and-Run

**Important**: Event monitoring is only available through the `aurora.run()` method. The auroraBuilder's `build_and_run()` convenience method does not support event parameters.

- **For event monitoring**: Use `.build()` then `aurora.run(inputs, event_callback=...)`
- **For simple execution**: Use `.build_and_run(inputs)` for convenience
- **For reusable workflows**: Use `.build()` once, then call `aurora.run()` multiple times

### Event Data Structure

Each event is an `auroraEvent` object with the following properties:

```python
@dataclass
class auroraEvent:
    event_type: auroraEventType          # Type of event
...
| `build()` | Build the aurora instance |
...
- `create_aurora()` - Returns a new `auroraBuilder` instance
...
result1 = await aurora.run(["Input 1"], event_callback=default_event_callback)
result2 = await aurora.run(["Input 2"])
```
...
result = await aurora.run(inputs)
```
...
### Before (Manual)
```python
from aurora_ai.aurora import aurora
from aurora_ai.aurora.memory import MessageMemory

# Manual construction
aurora = aurora(MessageMemory())
aurora.add_nodes([agent1, tool1, agent2])
aurora.start_at(agent1)
aurora.add_edge(agent1.name, [tool1.name])
aurora.add_edge(tool1.name, [agent2.name])
aurora.add_end_to(agent2)
aurora.compile()
...
aurora now supports intelligent routing using Large Language Models! Instead of writing complex routing logic, let an LLM analyze the conversation and decide which agent should handle the next step.

### Quick Example

```python
from aurora_ai.aurora import create_llm_router
