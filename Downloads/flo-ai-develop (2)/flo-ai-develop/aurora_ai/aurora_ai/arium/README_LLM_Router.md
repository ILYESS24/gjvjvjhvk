# LLM-Powered Routing for aurora Workflows

This document describes the LLM-powered routing functionality that enables intelligent, context-aware routing decisions in aurora workflows.
...
```python
from aurora_ai.aurora import create_llm_router, auroraBuilder
...
```python
from aurora_ai.aurora import llm_router
...
```python
from aurora_ai.aurora.llm_router import BaseLLMRouter
...
```python
import logging
logging.getLogger('aurora_ai').setLevel(logging.DEBUG)
...
For detailed API documentation, see the docstrings in `aurora_ai/aurora/llm_router.py`.
