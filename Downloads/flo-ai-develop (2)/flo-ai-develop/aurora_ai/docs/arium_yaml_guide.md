# aurora YAML Configuration Guide
...
The `auroraBuilder.from_yaml()` method allows you to define complex multi-agent workflows using declarative YAML configuration. This approach provides several benefits:
...
```python
from aurora_ai.aurora.memory import MessageMemory, CustomMemory

# Use default MessageMemory
builder = auroraBuilder.from_yaml(yaml_str=config)

# Or pass custom memory
custom_memory = MessageMemory()  # or your custom memory implementation
builder = auroraBuilder.from_yaml(yaml_str=config, memory=custom_memory)
```
...
try:
    builder = auroraBuilder.from_yaml(yaml_file="workflow.yaml")
...
```
ValueError: YAML must contain an "aurora" section
```
**Solution**: Ensure your YAML has the required `aurora` section.
