...
// Génère un ID unique stable (évite Math.random() qui est impur)
let matterBodyIdCounter = 0;
const generateMatterId = () => {
  matterBodyIdCounter += 1;
  return `matter-body-${matterBodyIdCounter}-${Date.now()}`;
};
...
const MatterBody = ({
  ...
  // Utilise un ID stable généré une seule fois au montage
  const idRef = useRef<string | null>(null)
  if (idRef.current === null) {
    idRef.current = generateMatterId();
  }
...
const Gravity = forwardRef<GravityRef, GravityProps>(
  (
    ...
    // Utilise une ref pour éviter les problèmes d'ordre de déclaration
    const updateElementsRef = useRef<(() => void) | null>(null)
    ...
    const initializeRenderer = useCallback(() => {
      ...
      if (autoStart) {
        ...
        // Utilise la ref pour éviter l'accès avant déclaration
        if (startEngineRef.current) {
