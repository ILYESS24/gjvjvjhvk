...
const ChatInputContext = React.createContext<ChatInputContextProps | undefined>(undefined);

// NOTE: useChatInputContext est gardé pour usage futur mais pas utilisé actuellement
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function _useChatInputContext() {
...
const SendButton = React.memo(({ 
  isDisabled,
}: Omit<SendButtonProps, 'textColor'>) => {
  return (
    <button
      type="submit"
...
const RippleEffects = React.memo(({ ripples, enabled }: RippleEffectsProps) => {
...
const InputArea = React.memo(({
  ...
  textColor,
  typedText
}: InputAreaProps & { typedText: string }) => {
  const textareaRef = React.useRef<HTMLTextAreaElement | null>(null);

  // Use typed text if no user input
  const displayValue = value || typedText;

  // Disable auto-resize - keep single line
  React.useEffect(() => {
    if (textareaRef.current) {
      textareaRef.current.style.height = "auto";
      textareaRef.current.style.height = "22px"; // Fixed single line height
    }
  }, [displayValue]);
  ...
  return (
    <div className="flex-1 relative h-full flex items-center">
      <textarea
        ...
        rows={1}
        className={`w-full min-h-[22px] max-h-[22px] bg-transparent text-sm font-normal text-left self-center text-white placeholder-gray-400 border-0 outline-none px-3 pr-10 py-1 z-20 relative resize-none overflow-hidden whitespace-nowrap`}
        style={{
...
// Menu Button component - gardé pour usage futur
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const _MenuButton = React.memo(({ 
  toggleMenu,
  menuRef,
  isMenuOpen,
  onSelectOption,
...
const ChatGPTInput = ({
  ...
  showEffects = true,
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  menuOptions: _menuOptions = ["Auto", "Max", "Search", "Plan"] as MenuOption[]
}: ChatInputProps) => {
  // State
  const [value, setValue] = React.useState("");
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [isMenuOpen, setIsMenuOpen] = React.useState(false);
  const [selectedOptions, setSelectedOptions] = React.useState<MenuOption[]>([]);
  const [ripples, setRipples] = React.useState<RippleEffect[]>([]);
  const [mousePosition, setMousePosition] = React.useState<Position>({ x: 50, y: 50 });

  // Auto-typing animation
  const autoTypingText = "Discover our new AI-powered editing features";
  ...
  React.useEffect(() => {
    if (isTyping && currentIndex < autoTypingText.length) {
      const timer = setTimeout(() => {
        setTypedText(prev => prev + autoTypingText[currentIndex]);
        setCurrentIndex(prev => prev + 1);
      }, 100); // Speed of typing animation

      return () => clearTimeout(timer);
    } else if (currentIndex >= autoTypingText.length) {
      setIsTyping(false);
    }
  }, [isTyping, currentIndex, autoTypingText]);

  // Initialize auto-typing when component mounts
  React.useEffect(() => {
    setIsTyping(true);
  }, []);
  ...
  const handleSubmit = React.useCallback(
    (e: React.FormEvent) => {
      e.preventDefault();
      const contentToSubmit = value.trim() || typedText.trim();

      if (contentToSubmit && onSubmit && !disabled) {
        onSubmit(contentToSubmit);
        setValue("");
      }
    },
    [value, typedText, onSubmit, disabled]
  );
  ...
  // Toggle menu - gardé pour usage futur avec menuOptions
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const _toggleMenu = React.useCallback(() => {
    setIsMenuOpen(prev => !prev);
  }, []);

  // Select option - gardé pour usage futur avec menuOptions
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const _selectOption = React.useCallback((option: MenuOption) => {
  ...
  // Remove option
  const removeOption = React.useCallback((option: MenuOption) => {
  ...
  // Check if submit is disabled - Allow submission with auto-typed text or user input
  const hasContent = value.trim() || typedText.trim();
  const isSubmitDisabled = disabled || !hasContent;

  // Calculate width classes - Allow expansion based on text length
  const hasModeSelected = selectedOptions.length > 0;
  const shouldExpandOnFocus = expandOnFocus && !hasModeSelected;
  const textLength = (value || typedText).length;

  // Dynamic width based on text length - More spacious expansion
  let baseWidthClass = "w-64"; // minimum width (increased from w-56)
  if (textLength > 20) baseWidthClass = "w-[26rem]";  // 416px for 20+ chars (increased)
  if (textLength > 35) baseWidthClass = "w-[32rem]";  // 512px for 35+ chars (increased)
  if (textLength > 45) baseWidthClass = "w-[40rem]";  // 640px for 45+ chars (increased)
  if (textLength > 55) baseWidthClass = "w-[48rem]";  // 768px for 55+ chars (increased)
  if (textLength > 65) baseWidthClass = "w-[56rem]";  // 896px for 65+ chars (increased)
  if (hasModeSelected) baseWidthClass = "w-[36rem]"; // override for mode selection

  const focusWidthClass = shouldExpandOnFocus ? "focus-within:w-[32rem]" : "";
