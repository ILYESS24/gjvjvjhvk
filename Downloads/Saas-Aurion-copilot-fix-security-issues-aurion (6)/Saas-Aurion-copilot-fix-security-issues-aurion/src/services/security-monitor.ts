...
export class SecurityMonitor {
  ...
  private async triggerCriticalAlert(event: SecurityEvent): Promise<void> {
    // Log critical alert to console and monitoring system
    ...
    // Send webhook notification (configurable via environment)
    const webhookUrl = typeof window !== 'undefined' 
      ? localStorage.getItem('aurion_alert_webhook') 
      : null;
    ...
    if (webhookUrl) {
      try {
        ...
        logger.info('[SecurityMonitor] Alert webhook sent successfully');
      } catch (webhookError) {
        logger.warn('[SecurityMonitor] Failed to send alert webhook', webhookError);
      }
    }
  }
