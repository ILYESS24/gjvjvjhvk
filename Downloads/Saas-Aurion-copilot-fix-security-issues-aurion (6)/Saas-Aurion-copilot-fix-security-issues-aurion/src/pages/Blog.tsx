...
const BlogPost = ({ title, excerpt, date, category, image }: BlogPostProps) => (
  <article className="group cursor-pointer">
    <div className="rounded-2xl overflow-hidden mb-6 h-64 bg-white/5 backdrop-blur-sm border border-white/10 relative">
      <img src={image} alt={title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
      <div className="absolute top-4 left-4 bg-black/80 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider text-white">
        {category}
      </div>
    </div>
    <div className="space-y-3">
      ...
      <Link to="#" className="inline-flex items-center gap-2 text-sm font-bold text-white hover:gap-3 transition-all pt-2">
...
export default function Blog() {
  return (
    <LegalLayout
      title="Blog"
      subtitle="Dernières actualités, tutoriels et insights de l'équipe AURION."
    >
